...
public class AlertClass {
    ...
    private static List<AlertClass> lista = new ArrayList<>();
    ...
    public static List<AlertClass> FilteresList (TipoAlerta _tipo){
        if (lista.isEmpty()){
            ...
            try {
                lista.add(new AlertClass(TipoAlerta.EMERGENCIA,new SimpleDateFormat("yyyy/MM/dd").parse("2017/08/08"),"nuevo terremoto de 5.6","Buscar un punto de ayuda cercano"));
        ...
        if (_tipo != null && _tipo != TipoAlerta.TODAS) {
            ...
            //sort(filter, Collections.<AlertClass>reverseOrder());
            return filter;
        }
        //sort(lista, Collections.<AlertClass>reverseOrder());
        return lista;
    }
}